#include <iostream>
#include "internalodessastepper.hpp"
...
namespace scigma
{
  namespace num
  {
    ...
    InternalOdessaStepper::InternalOdessaStepper(bool computeJacobian, double dt, const Function& tFunc, 
						 const VecF& xFuncs, const VecF& rhsFuncs, const VecF& funcFuncs, 
						 bool stiff, double aTol, double rTol, size_t maxIter):
      Stepper(computeJacobian,xFuncs.size(),funcFuncs.size(),tFunc.evaluate()),dt_(dt),tFunc_(tFunc), xFuncs_(xFuncs),rhsFuncs_(rhsFuncs),funcFuncs_(funcFuncs),
      odessa_(nVar,odessa_f(tFunc_,xFuncs_,rhsFuncs_),
	      odessa_dfdx(xFuncs_,rhsFuncs_),computeJacobian?nVar:0,NULL,stiff,aTol,rTol,maxIter),x_(odessa_.x()),jac_(odessa_.sensitivity())
    {
      odessa_.t()=t0_;
