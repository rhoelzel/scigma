...
SCENARIO ("autonomous external ODEs with parameters","[equationsystem][external]")
{
  std::srand(std::time(NULL));

  GIVEN("a vector of variable names (varNames), a vector of parameter names (parNames), the right hand side (f_p) of the ODE [optional: a Jacobian (dfdx_p), a function to evaluate the sensitivity (dfdp), a set of function names (funcNames), the associated evaluation routine (func_p)]")
    {
      VecS varNames;
	...
	REQUIRE_EXCEPTION((ExternalEquationSystem(VecS(),parNames,f_p)),"need at least one variable");

      WHEN("parNames is empty")
	THEN("exception \"need at least one parameter\" is thrown")
	REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,VecS(),f_p)),"need at least one parameter");
      ...
      WHEN("varNames, parNames and funcNames are well-formed")
	{
	  THEN("no exception \"ill-formed <type> name: <name>\" is thrown")
	      ...
	      for(int i(0);i<100;++i)
		{
		  ...
		  CAPTURE(varNames[1]);
		  REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p)),"");
		}
	      for(int i(0);i<100;++i)
		{
	      ...
	      for(int i(0);i<100;++i)
		{
		  ...
		  CAPTURE(funcNames[0]);
		  REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,funcNames, func_p)),"");
		}
	    }
	}
      ...
      WHEN("any name in varNames, parNames or funcNames is ill-formed")
	{
	  THEN("exception \"ill-formed <type> name: <name>\" is thrown")
	      ...
	      for(int i(0);i<100;++i)
		{
		  ...
		  CAPTURE(varNames[1]);
		  REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p)),"ill-formed variable name:");
		}
	      varNames[1]="y";
	      for(int i(0);i<100;++i)
		{
		  ...
		  CAPTURE(parNames[2]);
		  REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p)),"ill-formed parameter name:");
		}
	      parNames[2]="c";
	      for(int i(0);i<100;++i)
		{
		  ...
		  CAPTURE(funcNames[0]);
		  REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,funcNames, func_p)),"ill-formed function name:");
		}
	    }
	}
      WHEN("names in varNames/parNames/funcNames are not unique")
	{
	  THEN("exception \"name is not unique: <name>\" is thrown")
	    {
	      varNames[1]="x";
	      REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p)),"name is not unique: x");
	      varNames[1]="y";
	      parNames[0]="b";
	      REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p)),"name is not unique: b");
	      parNames[0]="a";
	      funcNames[2]="f_2";
	      REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,funcNames,func_p)),"name is not unique: f_2");
	      funcNames[2]="f_3";
	      parNames[2]="x";
	      REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,funcNames,func_p)),"name is not unique: x");
	      parNames[2]="c";
	      parNames[1]="f_3";
	      REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,funcNames,func_p)),"name is not unique: f_3");
	      parNames[1]="b";
	      varNames[0]="f_1";
	      REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,funcNames,func_p)),"name is not unique: f_1");
	    }
	}
	...
	{
	  ExternalEquationSystem eqsys(varNames,parNames,f_p);
	  REQUIRE(!eqsys.dfdx());
	  REQUIRE(!eqsys.dfdx_p());
	  REQUIRE(!eqsys.dfdx_pt());
	}

      WHEN("funcNames is non-empty and func_p is NULL")
	THEN("exception \"provided function names but no evaluation function\" is thrown")
	REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,funcNames)),"provided function names but no evaluation function");

      WHEN("vector funcNames is empty and func_p is not NULL")
	THEN("exception \"provided evaluation function but no function names\" is thrown")
	REQUIRE_EXCEPTION((ExternalEquationSystem(varNames,parNames,f_p,NULL,NULL,VecS(),func_p)),"provided evaluation function but no function names");
      ...
      WHEN("the ExternalEquationSystem has been successfully constructed with just varNames, parNames and f")
	{
	  ...
	  THEN("f_t() returns a wrapper of the original right hand side function f_p for current parameters with the additional parameter t")
	    {
	      eqsys.f_t()(0,x,rhs_2);
	      REQUIRE(rhs_1[0]==rhs_2[0]);
	      REQUIRE(rhs_1[1]==rhs_2[1]);
	    }
	  THEN("f_pt() returns a wrapper of the original right hand side function f_p with the additional parameter t")
	    {
	      eqsys.f_pt()(0,x,p,rhs_2);
	      REQUIRE(rhs_1[0]==rhs_2[0]);
	      REQUIRE(rhs_1[1]==rhs_2[1]);
	    }
	}
      WHEN("the Jacobian (dfdx_p) has been provided")
	{
	  ExternalEquationSystem eqsys(varNames,parNames,f_p,dfdx_p);
	  ...
	  THEN("dfdx() returns a wrapper the original Jacobian dfdx_p for current parameters")
	    {
	      eqsys.dfdx()(x,jac_2);
	  ...
	  THEN("dfdx_p() returns the original Jacobian")
	    {
	  ...
	  THEN("dfdx_t() returns a wrapper of the original Jacobian dfdx_p for the current parameters with the additional parameter t")
	    {
	  ...
	  THEN("dfdx_pt() returns a wrapper of the original Jacobian dfdx_p with the additional parameter t")
	    {
	      eqsys.dfdx_pt()(0,x,p,jac_2);
	      REQUIRE(jac_1[0]==jac_2[0]);
	      REQUIRE(jac_1[1]==jac_2[1]);
	      REQUIRE(jac_1[2]==jac_2[2]);
	      REQUIRE(jac_1[3]==jac_2[3]);
	...
	{
	  ExternalEquationSystem eqsys(varNames,parNames,f_p,dfdx_p,dfdp_p);
	  double x[]={5.4321,0.9876};
	  double p[]={-10,-11,-12};
	  eqsys.set("a",-10); eqsys.set("b",-11); eqsys.set("c",-12);
	  ...
	  THEN("dfdp_p() returns the original sensitivity function dfdx_p")
	    {
      ...
      WHEN("additional functions (funcNames, func_p) have been provided")
	{
	  ExternalEquationSystem eqsys(varNames,parNames,f_p,dfdx_p,dfdp_p,funcNames,func_p);
	  double x[]={9.8765,4.3210};
	  eqsys.set("x",9.8765); eqsys.set("y",4.3210);
	  double p[]={-10,-11,-12};
	  eqsys.set("a",-10); eqsys.set("b",-11); eqsys.set("c",-12);
	  double funcvals_1[3]; double funcvals_2[3];
	  func_p(x,p,funcvals_1);
	  ...
	  THEN("retrieving functions by name works")
	    {
	      REQUIRE(eqsys.get("f_1")==std::sin(9.8765+4.3210)*(-10));
	      REQUIRE(eqsys.get("f_2")==std::cos(9.8765+4.3210)*(-11));
	      REQUIRE(eqsys.get("f_3")==std::tan(9.8765+4.3210)*(-12));
	    }
	  THEN("function_values() returns the correct values")
	    {
	      REQUIRE(eqsys.function_values()[0]==std::sin(9.8765+4.3210)*(-10));
	      REQUIRE(eqsys.function_values()[1]==std::cos(9.8765+4.3210)*(-11));
	      REQUIRE(eqsys.function_values()[2]==std::tan(9.8765+4.3210)*(-12));
	    }
	  ...
	  THEN("func_t() returns a wrapper of the original function func_p for the current parameters with the additional parameter t")
	    {
	  ...
	  THEN("func_pt() returns a wrapper of the original function func_p with the additional parameter t")
	    {
	      eqsys.func_pt()(0,x,p,funcvals_2);
	      REQUIRE(funcvals_1[0]==funcvals_2[0]);
	      REQUIRE(funcvals_1[1]==funcvals_2[1]);
	      REQUIRE(funcvals_1[2]==funcvals_2[2]);
	    }
