...
namespace scigma
{
  namespace num
  {
    IntegrationStepper::IntegrationStepper(const EquationSystem& eqsys, double dt, bool stiff,
					   double aTol, double rTol, size_t maxIter):
      nVar_(eqsys.n_variables()),nFunc_(eqsys.n_functions()),t0_(eqsys.time()),
      odessa_(nVar_,eqsys.f_pt(),eqsys.dfdx_pt(),nVar_,NULL,stiff,aTol,rTol,maxIter),
      x_(odessa_.x()),jac_(odessa_.sensitivity()),func_t_(eqsys.func_t()),funcVals_(nFunc_),
      dt_(dt)
    {
      odessa_.t()=t0_;
