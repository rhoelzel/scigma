...
! ---------- ------
  SUBROUTINE AUTOAE(AC,ICP,ICU,FUNI,STPNAEI,FNCI)
...
! This is the entry subroutine for algebraic systems.
    ...
    IF(MPIIAM()>0)THEN
       ...
       ENDIF
    ENDIF
    CALL CNRLAE(AC,ICP,ICU,FUNI,STPNAEI,FNCI)

  END SUBROUTINE AUTOAE

! ---------- ------
  SUBROUTINE CNRLAE(AC,ICP,ICU,FUNI,STPNAEI,FNCI)

    USE IO
    USE MESH
    USE SUPPORT, ONLY: FNCS, STOPPED, INIT2, INIT3, &
         INITSTOPCNTS, PVLI, LBITP
    USE AUTO_TYPES, ONLY: NPARX
...
! Local
    ...
    CALL INIT3(AC,ICP,PAR,THL,THU,IUZ,VUZ)
    THU(AP%NDIM+1)=THL(1)

    ALLOCATE(AC%SUPPORT%P0V(NDIM+1,NDIM+1),U(NDIM+1),UDOT(NDIM+1))
    ALLOCATE(STUD(NBIFS,NDIM+1),STU(NBIFS,NDIM+1),TEST(AP%NTEST),&
         AC%SUPPORT%EVV(NDM))
    ALLOCATE(AC%SUPPORT%P1V(NDIM+1,NDIM+1))
    ...
    AC%SUPPORT%P1V(:,:)=0.d0
    DO I=1,NDIM+1
       AC%SUPPORT%P1V(I,I)=1.d0
    ENDDO

! Generate the starting point

    NODIR=1
    CALL STPNAEI(AC,PAR,ICP,U,UDOT,NODIR)
    CALL PVLI(AC,ICP,U,NDIM,PAR,FNCI)
...
! Write constants
    ...
    DO NBFC=0,NBFCS !bifurcation switch loop
       ...
       NTOT=0
       AP%NTOT=NTOT
       CALL INITSTOPCNTS(AC,ISP,ILP,ITPST,STOPCNTS)
       ISTOP=.FALSE.
       NIT=1
...
! Starting procedure  (to get direction vector) :
       ...
       IF(NODIR==1.AND.ISW>=0)THEN
          CALL STPRAE(AC,PAR,ICP,FUNI,U,UDOT,THU,0,AC%SUPPORT%P0V)
       ELSEIF(IRS/=0.AND.ISW<0)THEN
          CALL STPRAE(AC,PAR,ICP,FUNI,U,UDOT,THU,1,AC%SUPPORT%P0V)
       ELSEIF(ABS(IPS)==1.OR.IPS==11)THEN
          CALL STPRAE(AC,PAR,ICP,FUNI,U,UDOT,THU,-1,AC%SUPPORT%P0V)
       ENDIF
       IF(ABS(IPS)==1.OR.IPS==11)THEN
          ! Get stability
          TMP=FNCI(AC,ICP,U,NDIM,PAR,6,ATYPEDUM)
       ENDIF
...
! or for the starting point
       ...
       CALL PVLI(AC,ICP,U,NDIM,PAR,FNCI)
       CALL STPLAE(AC,PAR,ICP,ICU,U,UDOT,NIT,ISTOP)
...
! determine the second point on the bifurcating or original branch
          CALL STEPAE(AC,PAR,ICP,FUNI,RDS,AC%SUPPORT%P0V,U,UDOT,THU,NIT,ISW<0)
          IF(NIT>0)CALL PVLI(AC,ICP,U,NDIM,PAR,FNCI)
...
! Find the next solution point on the branch
          CALL STEPAE(AC,PAR,ICP,FUNI,RDS,AC%SUPPORT%P0V,U,UDOT,THU,NIT)
          ISTOP=NIT==0
          IF(.NOT.ISTOP)THEN
             CALL PVLI(AC,ICP,U,NDIM,PAR,FNCI)
          ENDIF
          ...
          ELSE
             ...
             DO ITEST=1,AP%NTEST
                ...
                CALL LCSPAE(AC,DSTEST,PAR,ICP,ITEST,FUNI,FNCI,AC%SUPPORT%P0V,&
          ...
          DO ITEST=1,ISTEPPED-1
             ...
             TEST(ITEST)=FNCS(AC,ICP,U,PAR,ATYPEDUM,IUZ,VUZ,ITEST,FNCI)
          ENDDO
...
! Adapt the stepsize along the branch
    ...
    DEALLOCATE(PAR,THL,THU,IUZ,VUZ,AC%SUPPORT%EVV,&
         AC%SUPPORT%P0V,AC%SUPPORT%P1V,U,UDOT,STUD,STU,TEST)
  END SUBROUTINE CNRLAE

! ---------- ------
  SUBROUTINE STPRAE(AC,PAR,ICP,FUNI,U,UDOT,THU,IPERP,AA)
...
! Finds the second point on the initial solution branch.
    ...
    TYPE(AUTOPARAMETERS), POINTER::AP
    INTEGER ICP(*),IPERP
    DOUBLE PRECISION U(AC%AP%NDIM+1),UDOT(AC%AP%NDIM+1),THU(*),PAR(*)
    DOUBLE PRECISION AA(AC%AP%NDIM+1,AC%AP%NDIM+1)
...
! Determine the direction of the branch at the starting point
    ...
    CALL FUNI(AC,NDIM,U,UOLD,ICP,PAR,2,F,DFDU,DFDP)
...
! ---------- ------
  SUBROUTINE STEPAE(AC,PAR,ICP,FUNI,RDS,AA,U,UDOT,THU,NIT,SW)
...
! known branch at the branch point.
    ...
    DOUBLE PRECISION, INTENT(OUT) :: AA(AC%AP%NDIM+1,AC%AP%NDIM+1)
...
! Local
    ...
    DO
       CALL SOLVAE(AC,PAR,ICP,FUNI,RDS,AA,U,UDOT,THU,NIT,BSW)
       IF(NIT>0)RETURN
...
! ---------- ------
  SUBROUTINE SOLVAE(AC,PAR,ICP,FUNI,RDS,AA,U,UDOT,THU,NIT,BSW)
...
!           U, UDOT, PAR are not updated
    ...
    DOUBLE PRECISION, INTENT(OUT) :: AA(AC%AP%NDIM+1,AC%AP%NDIM+1)
    DOUBLE PRECISION, INTENT(INOUT) :: U(AC%AP%NDIM+1),UDOT(AC%AP%NDIM+1),PAR(*)
...
! differential equation and its derivatives :
    ...
    DO NIT1=1,ITNW
       ...
       NIT=NIT1
       PAR(ICP(1))=U(NDIM+1)
       CALL FUNI(AC,NDIM,U,UOLD,ICP,PAR,2,RHS,DFDU,DFDP)
...
!
! ---------- ------
  SUBROUTINE LCSPAE(AC,DSTEST,PAR,ICP,ITEST,FUNI,FNCI,AA, &
...
! DS is less than the user-supplied toler EPSS.
    ...
    DOUBLE PRECISION, INTENT(INOUT) :: AA(AC%AP%NDIM+1,AC%AP%NDIM+1) 
    DOUBLE PRECISION, INTENT(INOUT) :: PAR(*),U(AC%AP%NDIM+1)
    DOUBLE PRECISION, INTENT(INOUT) :: UDOT(AC%AP%NDIM+1)
...
! Check whether FNCS has changed sign.
    ...
    Q0=Q
    Q1=FNCS(AC,ICP,U,PAR,ATYPE,IUZ,VUZ,ITEST,FNCI)
...
! If requested write additional output on unit 9 :
       ...
       CALL PVLI(AC,ICP,U,AP%NDIM,PAR,FNCI)

       Q=FNCS(AC,ICP,U,PAR,ATYPE,IUZ,VUZ,ITEST,FNCI)
...
! Return if relative tolerance has been met :
    ...
    DSTEST=DSTESTS
    CALL PVLI(AC,ICP,U,AP%NDIM,PAR,FNCI)
    Q=FNCS(AC,ICP,U,PAR,ATYPEDUM,IUZ,VUZ,ITEST,FNCI)
    DEALLOCATE(US,UDOTS,AAS)
...
! ---------- ------
  SUBROUTINE STPLAE(AC,PAR,ICP,ICU,U,UDOT,NIT,ISTOP)
...
! Determine stability and print output on units 6 and 7.
    ...
    ENDIF
    CALL WRLINE(AC,PAR,ICU,IBR,NTOTS,LABW,AMP,U)

! Write restart information for multi-parameter analysis :

    IF(LABW.NE.0)THEN
       CALL WRTSP8(AC,PAR,ICP,LABW,U,UDOT)
       AP%LAB=AP%LAB+1
    ENDIF
!
...
! ---------- ------
  SUBROUTINE WRTSP8(AC,PAR,ICP,LAB,U,UDOT)
